...
namespace SistVentas.BLL.Implementacion
{
    public class ProductoService:IProductoService
    {
        ...
        public async Task<bool> Eliminar(int idProducto)
        {
            try
            {
                Producto producto_encotrado = await _repositorio.Obtener(p => p.IdProducto == idProducto);

                if (producto_encotrado == null) 
                    throw new TaskCanceledException("El producto no Existe");

                string nombreImagen = producto_encotrado.NombreImagen;

                bool respuesta = await _repositorio.Eliminar(producto_encotrado);
